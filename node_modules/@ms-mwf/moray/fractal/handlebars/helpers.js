...
module.exports = function(fractal, version = '') {
  return {
    ...
    */
    interpolateString: (template, data) => template.replace(/%{(\w+)}/g, (match, key) => {
    ...
    compareOr: (val1, val2, val3, val4, opts) =>
      (val1 === val2 || val3 === val4) ? opts.fn(opts.data.root) : opts.inverse(opts.data.root),
    ...
    ifCond: (input1, operator, input2, options) => {
      switch (operator) {
        case '==':
          ...
          return (input1 == input2) ? options.fn(options) : options.inverse(options);
        case '===':
          return (input1 === input2) ? options.fn(options) : options.inverse(options);
        case '!=':
    ...
    */
    contains: (compareValue, searchValue) => compareValue.indexOf(searchValue) > -1,
    /**

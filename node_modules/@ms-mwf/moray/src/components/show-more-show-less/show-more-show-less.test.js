import ShowMoreShowLess, { Selector } from './show-more-show-less';
...
`('$name show more show less with no options passed in', ({ index }) => {
  ...
  beforeAll(() => {
    ...
    obj = instances.find(inst => {
      return inst.el.id === idMap[index].container;
    });
    objIds = idMap[index];
  ...
  describe('when the instance is removed', () => {
    ...
    it('should be removed from the instances array', () => {
      expect(
        instances.some(inst => {
          return inst.el.id !== idMap[index].toggleButton;
        })
      ).toBeTruthy();
    });
...
describe('Multi-element update tests', () => {
  ...
  describe('When a new element is inserted as the first toggleable element', () => {
    beforeAll(() => {
      ...
      instances = ShowMoreShowLess.getInstances();
      obj = instances.find(inst => {
        return inst.el.id === idMap[2].container && inst.toggleableContent && inst.visibleContent;
      });
      document

...
describe(Tab.name, () => {
  ...
  describe('update()', () => {
    ...
    it('should trigger the `onUpdate` custom event', () => {
      expect(tab1.el.dispatchEvent).toBeCalled();
      expect(tab1.el.dispatchEvent.mock.calls[0][0].type).toBe(EventName.ON_UPDATE);
    });

    it('should be able to be update the addUrlToHistory option', () => {
      const tab1 = new Tab({ el: document.getElementById('tab1-tab'), addUrlToHistory: true });
      expect(tab1.addUrlToHistory).toBe(true);
      tab1.update({
        addUrlToHistory: false
      });
      expect(tab1.addUrlToHistory).toBe(false);
    });
  });
});

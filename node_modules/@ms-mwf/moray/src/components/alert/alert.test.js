...
describe('Alert', () => {
  ...
  describe('close with preventDefault', () => {
    it('should cancel close if default prevented', () => {
      alert.el.dispatchEvent = jest.fn(e => e.preventDefault());
      alert.close();

      expect(alert.el.dispatchEvent).toBeCalled();
      expect(alert.el.dispatchEvent.mock.calls[0][0].type).toBe(EventName.CLOSE);
      expect(alert.el.dispatchEvent.mock.calls[0][0].defaultPrevented).toBe(true);
      expect(alert.el.classList).toContain(ClassName.SHOW);
      expect(alert.el.dispatchEvent.mock.calls.length).toBe(1);
    });
  });

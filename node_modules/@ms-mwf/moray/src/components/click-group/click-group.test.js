...
describe('Click group with preventDefault', () => {
  beforeAll(() => {
    // Set up our document body
    document.body.innerHTML = clickGroupHtml;
  ...
  it('should cancel click if default prevented', () => {
    instances[0].el.dispatchEvent = jest.fn(e => e.preventDefault());
    instances[0].el.click();

    expect(instances[0].el.dispatchEvent).toBeCalled();
    expect(instances[0].el.dispatchEvent.mock.calls[0][0].type).toBe(EventName.ON_CLICK);
    expect(instances[0].el.dispatchEvent.mock.calls[0][0].defaultPrevented).toBe(true);
    expect(instances[0].target.click).not.toBeCalled();
  });
});
